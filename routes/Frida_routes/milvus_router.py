...
async def upload_wiki_data_from_mysqldb_to_milvus(user_data: dict = Body(...)):  
    ...
    try:
        ...
        if not postgres.check_user_is_admin(user_id):
            logger.warning("User %s attempted to upload wiki data without admin rights", user_id)
            raise HTTPException(
        ...
        milvus_data_count, deleted_data_count = crud.insert_all_data_from_postgres_to_milvus()
