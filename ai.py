...
PROMPT_TEMPLATES = {
    ...
    4. Только эти теги HTML (<b>, <i>, <a>, <code>, <pre>) НЕЛЬЗЯ ИСПОЛЬЗОВАТЬ: <ul>, <br>, <table>, <small> и остальные!
    5. Обязательно укажи ссылку источника из какой статьи ты взял информацию, если есть.
    6. Если в запросе есть вопрос про тарифы, нужно предложить воспользоваться командой бота /tariff, но толкьо в то случае, если в именно последнем обращении был вопрос о тарифах.
    """
}
